...
const STORAGE_KEY = 'donde_ray_db_v2'
...
function loadDb() {
    try {
        ...
        if (raw) {
            ...
            if (parsed && Array.isArray(parsed.users) && Array.isArray(parsed.reservations)) {
                if (!Array.isArray(parsed.reviews)) parsed.reviews = initialDb.reviews ? [...initialDb.reviews] : []
                let changed = false
    ...
    const initial = {
        ...
        reviews: initialDb.reviews ? [...initialDb.reviews] : [],
    }
    try {
...
export async function mockFetch(input, init = {}) {
    ...
    const isMockTarget =
        ...
        urlString.includes('/api/reviews') ||
        urlString.startsWith('/reservations') ||
        urlString.startsWith('/users') ||
        urlString.startsWith('/reviews') ||
        /^(https?:\/\/[^/]+)?\/api\/(users|reservations|reviews)/i.test(urlString) ||
        /^(https?:\/\/[^/]+)?\/(users|reservations|reviews)/i.test(urlString)
    ...
    try {
        ...
        if (pathname === '/reviews' || pathname.startsWith('/reviews/')) {
            if (method === 'GET') {
                return new Response(JSON.stringify(db.reviews || []), { status: 200, headers: { 'Content-Type': 'application/json' } })
            }
            if (method === 'POST') {
                let body = {}
                try { body = typeof init.body === 'string' ? JSON.parse(init.body) : init.body || {} } catch { body = {} }
                const review = { id: body.id || `review-${Date.now()}`, ...body, createdAt: body.createdAt || new Date().toISOString() }
